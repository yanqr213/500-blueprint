...
进入 Home Assistant 的设备页面，确认外部电表作为一个设备存在。蓝图创建自动化时先选择 `Meter type / power formula`，再选择 `External meter device`。

如果是 Shelly Pro 3EM、EcoTracker 或 BitShake / Tasmota，通常只需要选择对应电表类型和电表设备，蓝图会从设备下自动寻找实时功率实体。

如果是普通 Shelly 3EM，建议选择 `Shelly 3EM L1 + L2 + L3 phase sum`，并在 `Shelly 3EM and custom meter entities` 中填写 L1、L2、L3 三相功率实体。

如果是自定义电表，需要选择对应的 `Custom formula`，并填写该公式需要的具体实体。

电表侧需要的是实时功率，不是累计电量。单位通常是 `W`，如果插件提供的是 `kW`，可在折叠的 `Advanced meter sign and unit settings` 中把 `External meter power multiplier` 设置为 `1000` 转成 `W`。
...
用户不需要自己把所有电表都改成这个方向。符号方向默认自动判断；如果现场方向反了，再展开 `Advanced meter sign and unit settings` 修改 `Meter sign convention`。
...
| 蓝图项 | 选择 |
| --- | --- |
| `Meter type / power formula` | 先选择电表类型或自定义功率公式 |
...
`Target grid power` 默认就是 `0 W`，普通用户不需要配置。高级写入节奏、方向、倍率等保持默认即可。

### 5.2 电表类型和需要填写的实体

蓝图不会根据电表类型动态隐藏字段。你只需要按所选类型填写对应实体，其余字段保持空即可。

| 电表或插件形态 | `Meter type / power formula` | 需要填写 |
| --- | --- | --- |
| Shelly Pro 3EM / EM / Pro EM 已有总功率实体 | `Shelly Pro 3EM / EM / Pro EM device total power` | 选择 `External meter device` |
| EcoTracker 已有当前功率实体 | `EcoTracker device current power` | 选择 `External meter device` |
| BitShake / Tasmota 提供当前功率实体 | `BitShake / Tasmota device current power` | 选择 `External meter device` |
| 普通 Shelly 3EM 只有 L1、L2、L3 三相实时功率 | `Shelly 3EM L1 + L2 + L3 phase sum` | 选择 L1、L2、L3 功率实体 |
| Shelly 3EM 提供每相 import/export 或 consumption/returned | `Shelly 3EM L1-L3 import/export formula` | 选择 L1/L2/L3 import 和 export 六个实体 |
| EcoTracker 功率在实体属性里 | `EcoTracker raw entity attribute formula` | 选择原始实体，并填写属性名，默认 `power` |
| 其他单个有符号总功率实体 | `Custom formula: signed total power` | 选择总功率实体 |
| 其他三相功率实体 | `Custom formula: L1 + L2 + L3` | 选择 L1、L2、L3 功率实体 |
| 其他总 import/export 功率对 | `Custom formula: export - import` | 选择总 import 和总 export 实体 |
| 其他三相 import/export 功率对 | `Custom formula: L1-L3 export - import` | 选择 L1/L2/L3 import 和 export 六个实体 |

`Auto-detect from meter device` 仍保留给已知设备形态，但新建自动化时更推荐手动选择明确的电表类型。
...
```text
Auto from selected meter type
```

通常建议先保持自动。

该设置默认隐藏在 `Advanced meter sign and unit settings` 中。如果现场发现方向反了，再展开手动改：
...
如果使用 import/export 公式，蓝图会直接按 `export - import` 计算，符号方向选项不会影响结果。
...
正常情况下不需要展开高级设置。蓝图会从 `SunEnergyXT device` 自动绑定一体机实体；电表侧按 `Meter type / power formula` 的说明填写即可。

只有以下情况才建议展开 `Advanced entity overrides`：

- 一体机实体自动绑定到了错误实体。
- 同一个一体机设备下有多个相似实体，且蓝图无法判断哪个是控制或反馈实体。

一体机高级覆盖项留空表示继续自动绑定；只填写需要纠偏的实体即可。电表实体不再放在高级覆盖里，而是放在 `Shelly 3EM and custom meter entities` 或 `Import/export meter entities` 中。
...
默认按所选电表类型判断方向并保持 `Auto from selected meter type` 时，通常不需要调整。
...
普通 Shelly 3EM 没有总功率实体时这样配置：

| 蓝图项 | 选择 |
| --- | --- |
| `Meter type / power formula` | `Shelly 3EM L1 + L2 + L3 phase sum` |
| `External meter device` | Shelly 3EM 电表设备 |
| `L1 / phase A power sensor` | Shelly L1 power |
| `L2 / phase B power sensor` | Shelly L2 power |
| `L3 / phase C power sensor` | Shelly L3 power |
| `Meter sign convention` | 先用 `Auto from selected meter type` |
...
如果电表提供每相 import/export 或 consumption/returned 实体，这样配置：

| 蓝图项 | 选择 |
| --- | --- |
| `Meter type / power formula` | `Shelly 3EM L1-L3 import/export formula` |
| `External meter device` | Shelly 3EM 电表设备 |
...
BitShake / Tasmota 通常只需要选择设备型电表类型：

| 蓝图项 | 选择 |
| --- | --- |
| `Meter type / power formula` | `BitShake / Tasmota device current power` |
| `External meter device` | Tasmota / BitShake 电表设备 |
| `Meter sign convention` | 先用 `Auto from selected meter type` |
...
| 蓝图项 | 选择 |
| --- | --- |
| `Meter type / power formula` | `EcoTracker device current power` |
| `External meter device` | EcoTracker 电表设备 |

如果 EcoTracker 功率在某个实体属性里：

| 蓝图项 | 选择 |
| --- | --- |
| `Meter type / power formula` | `EcoTracker raw entity attribute formula` |
| `Total/current power sensor` | EcoTracker 原始实体 |
| `Meter power attribute` | 功率属性名，默认 `power` |

### 11.5 自定义电表

自定义电表先选择一个公式，再填写公式需要的实体：

| 公式 | 需要填写 |
| --- | --- |
| `Custom formula: signed total power` | `Total/current power sensor` |
| `Custom formula: L1 + L2 + L3` | L1、L2、L3 三相功率实体 |
| `Custom formula: export - import` | 总 import 和总 export 实体 |
| `Custom formula: L1-L3 export - import` | L1/L2/L3 import 和 export 六个实体 |
...
| Shelly 3EM 数据不对 | `Meter type / power formula` 是否选成三相求和或 import/export，L1/L2/L3 实体是否填对 |
