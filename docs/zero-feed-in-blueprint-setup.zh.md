...
本蓝图通过外部电表实时功率闭环控制一体机的 `GS` 和 `IS`，实现零馈电或接近零馈电控制。蓝图不使用一体机的 `MM` 和 `MD`。
...
满电保持逻辑：SOC 达到上限后进入满电保持，SOC 低于 `上限 - 回差` 后退出。

低 SOC 保持逻辑：SOC 达到下限后进入低 SOC 保持，SOC 高于 `下限 + 回差` 后退出。
...
| 配置项 | 默认值 | 含义 |
| --- | --- | --- |
| `GS write resolution` | `1 W` | GS 最小写入变化 |
| `IS write resolution` | `10 W` | IS 最小写入变化 |
